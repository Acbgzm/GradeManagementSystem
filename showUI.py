...
def LogIn(ID, PW, isAdmin):
    if isAdmin == 1:
        print(ID, PW)
        if ExitAdmin(ID, PW) == True:
            gradeUI.AdminUI(ID)
        else:
            WrongUI()
    else:
        if ExitUser(ID, PW) == True:
            gradeUI.UserUI(ID)
        else:
            WrongUI()
...
def RegUI():
    ...
    win_reg.geometry('%dx%d+%d+%d' % (width,height,(win_reg.winfo_screenwidth() - width ) / 2, (win_reg.winfo_screenheight() - height) / 2))
    ...
    btnCheck = Checkbutton(win_reg, text='管理员登陆', variable = isAdmin, onvalue = 1, offvalue = 0)
